...
const PaymentLoansInfo = ({ setAuth }) => {
  ...
  const [name, setName] = useState(""); // State for client's name
  ...
  const getClient = async () => {
    try {
      ...
      const parseRes = await response.json();
      setName(`${parseRes.firstName} ${parseRes.lastName}`); // Set client's name
    } catch (error) {
  ...
  useEffect(() => {
    GetLoans();
    getClient(); // Fetch client details when component mounts
  }, []);
            ...
            <tbody>
              {loans.length <= 0 ? (
                ...
                <tr>
                  ...
                  <td className="border px-4 py-2 ">KES {loans.gross_loan}</td>
                  <td className="border px-4 py-2 ">KES {loans.balance}</td>
