...
const LoanInfo = () => {
  ...
  // Delete loan function with redirection
  async function deleteLoan(id) {
    try {
      ...
      setTimeout(() => {
        ...
        navigate("/borrowers"); // Redirect to /borrowers page after deletion
      }, 2000);
    } catch (error) {
      console.log(error.message);
    }
                    ...
                    <tr key={index}>
                      ...
                      <td className="border px-4 py-2 flex">
                        ...
                        <button className="bg-red-500 hover:bg-red-700 text-white font-bold h-10 px-4 rounded focus:outline-none focus:shadow-outline w-full ">
                          <Link to={`/payment/${loan.clientId}/${loan.loanId}`}>
                            KES
                          </Link>
                        </button>
                      </td>
